    ...
    private static WeaponData DualSwordSetter(Player player){
        ...
        if (!mainHandItem.isEmpty() && !offHandItem.isEmpty()) {
            ...
            if (mainHandClaw && offHandClaw) {
                float default_cooldown = (float) player.getAttributeValue(SwordSkillAttribute.COOLDOWN_ATTRIBUTE);
                Objects.requireNonNull(player.getAttribute(SwordSkillAttribute.COOLDOWN_ATTRIBUTE)).setBaseValue(default_cooldown-0.5);
