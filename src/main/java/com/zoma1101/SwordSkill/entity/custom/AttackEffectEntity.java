...
public class AttackEffectEntity extends Entity {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide) {
            String particleName = this.getSkillParticle();
            if (particleName != null && particleName.startsWith("spear")) {
                applyRayDamage();
            } else {
    ...
    private boolean isEntityInRay(LivingEntity entity, Vec3 rayStart, Vec3 rayEnd) {
        // 当たり判定を緩和するため、エンティティのボウンディングボックスを半径1.5ブロック分広げる
        AABB entityAABB = entity.getBoundingBox().inflate(1.5D);
        return entityAABB.clip(rayStart, rayEnd).isPresent();
    }
