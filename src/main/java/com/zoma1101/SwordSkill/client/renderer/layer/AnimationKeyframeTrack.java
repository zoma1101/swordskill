...
public class AnimationKeyframeTrack {
    ...
    public Vector3f evaluate(float t) {
        ...
        if (span <= 0f) return new Vector3f(prev.value());
    ...
    private static float applyEasing(float t, String easing) {
        ...
        return switch (easing) {
            ...
            case "easeOutCubic"  -> { float f = t - 1f; yield f * f * f + 1f; }
            case "easeInQuart"   -> t * t * t * t;
            case "easeOutQuart"  -> { float f = t - 1f; yield 1f - f * f * f * f; }
    ...
    public record AnimationData(AnimationKeyframeTrack armRotTrack, AnimationKeyframeTrack armPosTrack,
                                AnimationKeyframeTrack bodyRotTrack, float animationLength) {
         ...
         */
            public AnimationData getMirrored() {
                ...
                return new AnimationData(mirroredArmRot, mirroredArmPos, mirroredBodyRot, this.animationLength);
            }
             ...
             */
            private AnimationKeyframeTrack mirrorTrack(AnimationKeyframeTrack original, boolean invertRot, boolean invertPos) {
                ...
                try {
                    ...
                    try (InputStream is = rm.getResourceOrThrow(loc).open();
                        ...
                        Map.Entry<String, JsonElement> firstEntry =
                                animations.entrySet().iterator().next();
                        ...
                        return new AnimationData(armRot, armPos, bodyRot, animLength);
                    }
                } catch (Exception e) {
                    return null;
                }
            ...
            private static AnimationKeyframeTrack parseTrack(JsonObject bones,
                ...
                if (!bones.has(boneName)) return track;

                JsonObject bone = bones.getAsJsonObject(boneName);
                if (!bone.has(trackName)) return track;
                ...
                for (Map.Entry<String, JsonElement> entry : trackObj.entrySet()) {
                    try {
                        ...
                        Vector3f vec = parseVector(kf.getAsJsonArray("vector"));
